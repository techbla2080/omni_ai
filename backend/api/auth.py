...
# Dedicated redirect URI for "Sign in with Google" — kept separate from Gmail/Calendar.
GOOGLE_LOGIN_REDIRECT_URI = getattr(settings, 'GOOGLE_LOGIN_REDIRECT_URI', '') or "https://omniai.biz/api/v1/auth/google/callback"

# Where to send the user after a successful / failed Google login
FRONTEND_URL = getattr(settings, 'FRONTEND_URL', '') or "https://omniai.biz"
...
async def login(request: LoginRequest, db: AsyncSession = Depends(get_db)):
    ...
    # user[1] (password_hash) can be NULL for Google-only accounts
    if not user or not user[1] or not verify_password(request.password, user[1]):
...
async def change_password(
    ...
    if not user or not user[0] or not verify_password(body.current_password, user[0]):
...
async def delete_account(
    ...
    if not user or not user[0] or not verify_password(body.password, user[0]):
...
async def google_login():
    """Redirect the user to Google's sign-in consent screen."""
...
async def google_callback(code: str = None, error: str = None, db: AsyncSession = Depends(get_db)):
    """Handle the Google callback, then redirect back to the app carrying the JWT."""
    ...
    if error or not code:
        return RedirectResponse(f"{FRONTEND_URL}/login.html?error=google_auth_failed")
    ...
    if token_response.status_code != 200:
        return RedirectResponse(f"{FRONTEND_URL}/login.html?error=token_exchange_failed")
    ...
    if not email:
        return RedirectResponse(f"{FRONTEND_URL}/login.html?error=no_email")
    ...
    if user:
        user_id = str(user[0])
        # Backfill google_id / avatar if this account was first made with email+password
        await db.execute(
            text("""
                UPDATE users
                ...
                WHERE id = :id
            """),
            {"gid": google_user.get("id"), "avatar": google_user.get("picture"), "id": user_id},
        )
        await db.commit()
    else:
        user_id = str(uuid.uuid4())
    ...
    # Issue our own JWTs and redirect back to the app.
    # Tokens go in the URL fragment (#...) so they are never sent to the server or written to logs.
    access_token = create_access_token(user_id)
    refresh_token_jwt = create_refresh_token(user_id)
    redirect_url = f"{FRONTEND_URL}/login.html#access_token={access_token}&refresh_token={refresh_token_jwt}"
